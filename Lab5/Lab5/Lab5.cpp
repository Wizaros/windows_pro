...
#include <bitset>
using namespace std;
extern "C" unsigned int IntegerShift_(unsigned int a, unsigned int count, unsigned int* a_shl, unsigned int* a_shr);
...
int main()
{
	...
	PrintResult("Example 1", rc, a, count, a_shl, a_shr);
	a = 0x00800080;
	count = 4;
	rc = IntegerShift_(a, count, &a_shl, &a_shr);
	PrintResult("Example 2", rc, a, count, a_shl, a_shr);
	a = 0x80000001;
	count = 31;
	rc = IntegerShift_(a, count, &a_shl, &a_shr);
	PrintResult("Example 3", rc, a, count, a_shl, a_shr);
	a = 0x55555555;
	count = 32;
	rc = IntegerShift_(a, count, &a_shl, &a_shr);
	PrintResult("Example 4", rc, a, count, a_shl, a_shr);
	return 0;
}
